...
async def handle_user_id_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.effective_message.reply_text("请输入有效的用户 ID\n/cancel 取消")
        return 0
    ...
    if not user:
        await update.effective_message.reply_text("用户 ID 不存在")
        return ConversationHandler.END

    keyboard = [
        [InlineKeyboardButton("切换用户组", callback_data=f"set_group|{user_id}")],
